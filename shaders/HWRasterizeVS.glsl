...
U_GlobalConstants;
layout(std430, binding = 1) readonly buffer FClusterPageData { uint mData[]; }
ClusterPageData;
layout(std430, binding = 2) readonly buffer FVisibleClusterSHWH { uint mData[]; }
VisibleClusterSHWH;
...
ClusterInfo GetClusterInfo(uint inPageIndex, uint inClusterIndex)
{
    ...
    uvec4       lodBounds                   = uvec4(ClusterPageData.mData[clusterBaseOffset + 2u],
...
void main()
{
    ...
    vec3        positionMS = uintBitsToFloat(uvec3(ClusterPageData.mData[currentVertexPositionDataOffset],
