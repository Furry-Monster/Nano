...
layout(std430, binding = 0) buffer FBVHBuffer { uint mData[]; }
BVHBuffer; // 21
layout(std430, binding = 1) buffer FEchoBuffer { uint mData[]; }
EchoBuffer;
layout(std430, binding = 2) buffer FMainAndPostNodeAndClusterBatches { uint mData[]; }
MainAndPostNodeAndClusterBatches;
layout(std430, binding = 3) buffer FCurrentWorkArgs { uint mData[]; }
CurrentWorkArgs;
layout(std430, binding = 4) buffer FNextWorkArgs { uint mData[]; }
NextWorkArgs;
...
FHierarchyNodeSlice UnpackHierarchyNodeSlice(uvec4 RawData0, uvec4 RawData1, uvec4 RawData2, uint RawData3)
{
    ...
    Node.ChildStartReference =
        Misc1.w; // When changing this, remember to also update StoreHierarchyNodeChildStartReference
    ...
    Node.StartPageIndex = BitFieldExtractU32(
        Misc2, NANITE_MAX_RESOURCE_PAGES_BITS, NANITE_MAX_CLUSTERS_PER_GROUP_BITS + NANITE_MAX_GROUP_PARTS_BITS);
...
FHierarchyNodeSlice GetHierarchyNodeSlice(uint NodeIndex, uint ChildIndex)
{
    ...
    const uvec4 RawData0 =
        uvec4(BVHBuffer.mData[i], BVHBuffer.mData[i + 1], BVHBuffer.mData[i + 2], BVHBuffer.mData[i + 3]);
    i = BaseAddress + 16 + ChildIndex * 4;
    const uvec4 RawData1 =
        uvec4(BVHBuffer.mData[i], BVHBuffer.mData[i + 1], BVHBuffer.mData[i + 2], BVHBuffer.mData[i + 3]);
    i = BaseAddress + 32 + ChildIndex * 4;
    const uvec4 RawData2 =
        uvec4(BVHBuffer.mData[i], BVHBuffer.mData[i + 1], BVHBuffer.mData[i + 2], BVHBuffer.mData[i + 3]);
                ...
                else
                {
                    if (currentSliceMipLevel == U_GlobalContants.mMisc0.x)
                    {
                        ...
                        for (uint i = 0u; i < clusterCountInLeafNode; i++)
                        {
                            ...
                            MainAndPostNodeAndClusterBatches.mData[1024 + clusterOutputOffset * 2 + 1] =
                                clusterOffsetInPage + i;
                            clusterOutputOffset++;
                        }
                    }
                }
