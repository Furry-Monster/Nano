...
static constexpr int kMipLevelCount = sizeof(kAvailableMipLevels) / sizeof(kAvailableMipLevels[0]);
...
void InitScene(int canvasWidth, int canvasHeight) {
    ...
    sProjectionMatrix.Perspective(90.0f, static_cast<float>(canvasWidth) / canvasHeight, 10.0f,
                                  10000.0f);
    ...
    float viewToPixels = 0.5f * sProjectionMatrix.v[5] * static_cast<float>(canvasHeight);
    ...
    sGlobalConstantsData.SetMisc0(kAvailableMipLevels[sCurrentMipLevelIndex], 0, 0, 0);
    ...
    sGlobalConstantsData.SetCameraViewDirectionWS(viewDir.x, viewDir.y, viewDir.z, sLODScaleHW);
...

    {
        ...
        sVisualizationTexture->mImageView =
            GenImageView2D(sVisualizationTexture->mImage, sVisualizationTexture->mFormat,
    ...
    sVisBuffer64 = GenBufferObject(
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, nullptr,
        canvasWidth * canvasHeight * static_cast<int>(sizeof(uint64_t)));
    ...
    sMainAndPostNodeAndClusterBatches =
        GenBufferObject(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT,
                        nullptr, kBufferSize4MB);
    SetObjectName(VK_OBJECT_TYPE_BUFFER, sMainAndPostNodeAndClusterBatches->mBuffer,
    ...
    // Init pass
    {
        ...
        sInitPass->SetComputeDispatchArgs(
            ...
            static_cast<int>(std::ceil(static_cast<float>(canvasHeight) / 8.0f)), 1);
        sInitPass->Build();
    }
    ...
    for (int i = 0; i < 4; i++) {
        ...
        sNodeAndClusterCullPasses[i] = new RenderPass(RenderPassType::Compute, name);
    ...
    // Full-screen quad for swapchain blit
    {
        ...
        mesh->mVBO = GenBufferObject(VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                     VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, mesh->mVertexData,
                                     sizeof(StaticMeshVertex) * 4);
        ...
        mesh->mMaterial.SetTexture2D(2, 0, sVisualizationTexture->mImageView, GenSampler());
    }
...
void RenderOneFrame(float /*frameTime*/) {
    ...
    BeginCommandBuffer(cb, VK_COMMAND_BUFFER_USAGE_ONE_TIME_SUBMIT_BIT);
    {
        ...
        sFSQNode->Draw(cb, GetSwapChainRenderPass(), sProjectionMatrix, sViewMatrix);
    }
