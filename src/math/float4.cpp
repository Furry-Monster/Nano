...
#include <math.h>
#include <string.h>
void float4::operator=(const float4& inV) { memcpy(v, inV.v, sizeof(v)); }
...
float  MinValue(float inA, float inB) { return inA < inB ? inA : inB; }
float  MaxValue(float inA, float inB) { return inA > inB ? inA : inB; }
float4 Min(const float4& inA, const float4& inB)
{ // component wise
    return float4(MinValue(inA.x, inB.x), MinValue(inA.y, inB.y), MinValue(inA.z, inB.z), MinValue(inA.w, inB.w));
}
float4 Max(const float4& inA, const float4& inB)
{ // component wise
    return float4(MaxValue(inA.x, inB.x), MaxValue(inA.y, inB.y), MaxValue(inA.z, inB.z), MaxValue(inA.w, inB.w));
}
...
float dot3(const float4& inA, const float4& inB) { return inA.x * inB.x + inA.y * inB.y + inA.z * inB.z; }
