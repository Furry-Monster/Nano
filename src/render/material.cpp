...
void Material::Init(const char *vsPath, const char *fsPath) {
  ...
  vkCreateDescriptorPool(GetVulkanDevice(), &poolInfo, nullptr,
                         &mDescriptorPool);
...
void Material::SetTexture2D(int binding, int dstArrayIndex,
...
void Material::Active(VkCommandBuffer cb, VkRenderPass renderPass) {
  if (mPSO == VK_NULL_HANDLE) {
    mPSO = CreatePSO(renderPass, mPrimitiveType, StaticMesh::sVertexBindings,
                     StaticMesh::sVertexAttributes, mVertexShader,
                     mFragmentShader);
  }
  ...
  vkCmdBindDescriptorSets(cb, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          params->mPipelineLayout, 0, 1, &mDescriptorSet, 0,
                          nullptr);
}
