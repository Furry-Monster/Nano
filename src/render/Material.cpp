...
void Material::Init(const char* inVSPath, const char* inFSPath)
{
    ...
    vkCreateDescriptorPool(GetVulkanDevice(), &descriptorPoolCreateInfo, nullptr, &mDescriptorPool);
    ...
    vkAllocateDescriptorSets(GetVulkanDevice(), &descriptorSetAllocateInfo, &mDescriptorSet);
}
...
void Material::Active(VkCommandBuffer inCommandBuffer, VkRenderPass inRenderPass)
{
    if (mPSO == nullptr)
    {
        ...
        if (mPSOType == 0)
        {
            mPSO = CreatePSO(inRenderPass,
                             mPrimitiveType,
                             StaticMesh::mVertexInputBindingDescriptions,
        ...
        {
            mPSO = CreatePSOVTF(inRenderPass,
                                StaticMesh::mVertexInputBindingDescriptions,
                                StaticMesh::mVertexInputAttributeDescriptions,
                                mVertexShader,
                                mTCSShader,
